...
btnTrees.addEventListener('click',
    async ()=>{
        let response= await fetch("arboles_AlamosNorte.geojson");
...
let btnAccidents= document.getElementById("btnAccidents");

btnAccidents.addEventListener('click',
    async ()=>{
        let response= await fetch("Siniestros_AlamosNorte.geojson");
        let datos= await response.json();
        //Agregar la capa al mapa
        L.geoJSON(
            datos,
